...
def extract_table_cells(img: np.ndarray) -> List[List[str]]:
    """
    OCR→DICT, then try in order:
      1) Header-based splits
      2) K-Means on word centers
      3) Grid-line detection by morphology
      4) Equal-width fallback
      5) If still no table, call parse_fallback()
    """
    ...
    for ln, idxs in lines.items():
        ...
        found = sum(
            ...
            if any(lbl in w for w in words)
        )
        if found >= 4:
            header_ln = ln
            break
    ...
    # 3) Grid-line detection fallback
    def detect_grid_splits(bin_img):
        inv = 255 - bin_img
        kern = cv2.getStructuringElement(
            cv2.MORPH_RECT, (1, bin_img.shape[0] // 40)
        )
    ...
    # bucket every word into (page,line) + column
    ...
    # if still nothing or <4 cols, fallback to regex
    if not table or all(len(r) < 4 for r in table):
        from utils import parse_fallback  # avoid circular import
...
def parse_fallback(img: np.ndarray) -> List[List[str]]:
    """
    OCR full page as text → split lines → regex on “low-high” ranges
    """
...
def parse_row(cells: List[str]) -> Optional[dict]:
    """Given [name, raw_val, unit, raw_range], return dict or None."""
    if len(cells) < 4:
        return None
    name, raw_val, unit, raw_range = [c.strip() for c in cells[:4]]
